...
(function (factory) {
    ...
    $.extend($.fn.modal.Constructor.prototype, {
        ...
        getUrl: function (element) {
            if (similarSong) {
                return similarSong.image.replace('170x170', '600x600');
            }
            return element.href || $(element).data('href');
        },
        getPreviewUrl: function (element) {
            if (similarSong) {
                return similarSong['preview-url'];
            }
            return element.getAttribute('preview-url');
        },
        getSongId: function (element) {
            if (similarSong) {
                return similarSong['song-id'];
            }
            return element.getAttribute('song-id');
        },
        getSimilars: function (element) {
            if (similarSong && similarSong.similars) {
                return similarSong.similars.join(',');
            }
            return element.getAttribute('similars').split(',');
        },
        ...
        loadImage: function () {
            var $this = this,
                ...
                previewUrl = this.getPreviewUrl(this.$links[index]),
                songId = this.getSongId(this.$links[index]),
                similars = this.getSimilars(this.$links[index]),
                oldImg;
            this.abortLoad();
            this.stopSlideShow();
            ...
            if (similarSong) {
                modal.find('.modal-title').text(similarSong.title + ' / ' + similarSong.artist);
            } else {
            ...
            $(audio).bind("ended", function() {
                ...
                if (similars instanceof Array) {
                    candidates = similars.filter(function(e) {
                        return played.indexOf(e) == -1;
                    });
                    if (candidates.length > 0) {
                        console.log('http://localhost:3000/api/song/' + candidates[0]);
                        $.ajax({
                            url: 'http://localhost:3000/api/song/' + candidates[0],
                            dataType: 'json',
                            async: false,
                        }).done(function (data) {
                            similarSong = data;
                            console.log(similarSong);
                        });
                    } else {
                        similarSong = null;
                        $this.preloadImages();
